...
{
    return MyPlugin_DllEntryPoint(hinst, reason, lpReserved);
}
...
PREFUNCDEF BOOL EFuncName_DoInit(const char* fullfilename, void* thisptr, const char* /*hostname*/, const char* /*hostversionstring*/, Fp_GlobalPluginCallback_GetStrVal incallbackfp_get_strval, Fp_GlobalPluginCallback_SetStrVal incallbackfp_set_strval)
{
    ...
    util::String::copyString(dlldir, MAX_PATH, fullfilename);
    char* rpathchar = strrchr(dlldir, '\\');
    if(rpathchar != NULL)
        *(rpathchar + 1) = '\0';
    // icon absolute path
    util::String::copyString(iconfpath, MAX_PATH, dlldir);
...
PREFUNCDEF BOOL EFuncName_GetStrVal(const char* varname, char* destbuf, int maxlen)
{
    ...
    {
        util::String::copyString(destbuf, maxlen, ThisPlugin_DisplayName);
        return TRUE;
    }
    if(strcmp(varname, DEF_FieldName_Author) == 0)
    {
        util::String::copyString(destbuf, maxlen, ThisPlugin_Author);
        return TRUE;
    }
    if(strcmp(varname, DEF_FieldName_UpdateURL) == 0)
    {
        util::String::copyString(destbuf, maxlen, ThisPlugin_UpdateURL);
        return TRUE;
    }
    if(strcmp(varname, DEF_FieldName_HomepageURL) == 0)
    {
        util::String::copyString(destbuf, maxlen, ThisPlugin_HomepageURL);
        return TRUE;
    }
    if(strcmp(varname, DEF_FieldName_ShortDescription) == 0)
    {
        util::String::copyString(destbuf, maxlen, ThisPlugin_ShortDescription);
        return TRUE;
    }
    if(strcmp(varname, DEF_FieldName_LongDescription) == 0)
    {
        util::String::copyString(destbuf, maxlen, ThisPlugin_LongDescription);
        return TRUE;
    }
    ...
    {
        util::String::copyString(destbuf, maxlen, ThisPlugin_AdvConfigString);
        return TRUE;
    }
    // Do we support a readme? Return "" if not
    if(strcmp(varname, DEF_FieldName_ReadMeString) == 0)
    {
        util::String::copyString(destbuf, maxlen, ThisPlugin_ReadMeString);
        return TRUE;
    }
    ...
    {
        util::String::copyString(destbuf, maxlen, ThisPlugin_IconFilename);
        return TRUE;
    }
...
void ReportErrorToHost(char *estring)
{
    ...
    if(callbackfp_set_strval != NULL && hostptr != NULL)
